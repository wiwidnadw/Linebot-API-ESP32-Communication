...
class Linebot {
	...
	public function push($body){
   	 ...
   	 public function pushImage64($to, $imageUrl, $previewImageUrl = false){
        	$body = array(
		    'to' => $to,
		    'messages' => [
			array(
			    'type' => 'image',
			    'originalContentUrl' => base64_decode($imageUrl),
			    'previewImageUrl' => base64_decode($previewImageUrl) ? base64_decode($previewImageUrl) : base64_decode($imageUrl)
			)
		    ]
		);
		$this->push($body);
